...
maior.\footnote{({\textbf{???}})} Os dois principais algoritmos
conhecidos são os: \emph{bagging}\footnote{({\textbf{???}})} e
\emph{boosting}\footnote{({\textbf{???}})}.

\hypertarget{bagging}{%
\subsubsection{Bagging}\label{bagging}}

O Random Forest é um algoritmo de Machine Learning que tem como base o
\emph{bootstrap aggregating}.\footnote{({\textbf{???}})}
