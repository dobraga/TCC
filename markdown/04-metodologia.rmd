...
Um problema de otimização pode ser definido como:^[@chong2013introduction]
...
Esse é um caso de otimização onde busca-se o maior valor possível da função objetivo $f$.

Onde $\boldsymbol{f_i}$ são as funções de restrição, $\boldsymbol{b_i}$ são as constantes que definem os limites, $\boldsymbol{x} = (x_1,...,x_n)$ é o vetor da variável a ser otimizada, 
...
![Um exemplo do uso da otimização bayesiana em um problema de design de brinquedo 1D.\label{bo}](fig/bo.png){height=50%}
